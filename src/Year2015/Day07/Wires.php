    ...
    private function wireInput(Generator $input): void
    {
        ...
        foreach ($input as $line) {
            if (preg_match('/^(?P<in_01>\d+) -> (?P<out>[a-z]+)/', $line, $matches)) {
                ...
                $this->wires->set($matches['out'], fn(): int => ~(int) $this->wires->get($matches['in_01']));
            } elseif (preg_match('/^(?P<in_01>\d+) AND (?P<in_02>[a-z]+) -> (?P<out>[a-z]+)/', $line, $matches)) {
                $this->wires->set($matches['out'], fn(): int => (int) $matches['in_01'] & $this->wires->get($matches['in_02']));
            } elseif (preg_match('/^(?P<in_01>[a-z]+) AND (?P<in_02>[a-z]+) -> (?P<out>[a-z]+)/', $line, $matches)) {
                $this->wires->set($matches['out'], fn(): int => $this->wires->get($matches['in_01']) & $this->wires->get($matches['in_02']));
                ...
                $this->wires->set($matches['out'], fn(): int => ($this->wires->get($matches['in_01']) << (int) $matches['in_02']));
                ...
                $this->wires->set($matches['out'], fn(): mixed => $this->wires->get($matches['in_01']));
            }
        }
    }
}
